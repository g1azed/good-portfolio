...
export const createRenderer = (rendererProps = {}, configureRenderer = (renderer) => {}) => {
    const renderer = new THREE.WebGLRenderer(rendererProps, { antialias: true, logarithmicDepthBuffer: true });
...
export const createComposer = (renderer, scene, camera, extraPasses) => {
    if (!renderer.capabilities.isWebGL2) {
        console.warn("WebGL2가 활성화되지 않았습니다. 후처리 효과가 비정상적으로 동작할 수 있습니다.");
    }

    const renderTarget = new WebGLRenderTarget(window.innerWidth, window.innerHeight, {
        format: THREE.RGBAFormat, // 색상 포맷 지정
    ...
    if (typeof extraPasses === "function") {
